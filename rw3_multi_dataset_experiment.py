"""
RW3多数据集预实验：验证OOD同配性模式的普遍性（完全自动化版）

支持数据集：
1. CLINC150 - 自动下载
2. Banking77-OOS - 自动下载
3. ROSTD - 自动下载（多种方式可选）
4. ToxiGen - 自动下载
...
print("="*80)
print("RW3多数据集预实验：OOD同配性模式普遍性验证（完全自动化版）")
print("="*80)
...
GLOBAL_CONFIG = {
    ...
    'output_dir': 'rw3_multi_dataset_results'
}
...
def load_banking77_data(config):
    ...
    try:
        ...
        id_labels = unique_labels[:50]
        ood_labels_set = unique_labels[50:]
...
def load_rostd_data(config):
    ...
    try:
        ...
        try:
            ...
            print(f"✅ ROSTD加载完成（HuggingFace Hub）:")
            ...
            print(f"⚠️ 方式1失败: {e1}")
            print("尝试方式2：GitHub直接下载...")

        # ==================================================================
        # 方式2：从GitHub下载原始TSV文件
        # ==================================================================
        try:
            # OOD数据从GitHub下载
            ood_url = "https://raw.githubusercontent.com/vgtomahawk/LR_GC_OOD/master/data/fbrelease/ood_train.tsv"

            print(f"下载OOD数据: {ood_url}")
            response = requests.get(ood_url, timeout=30)
            ...
            for line in lines:
                fields = line.split('\t')
                if len(fields) >= 3:
                    ood_texts.append(fields[2])

            # 采样OOD
            np.random.seed(GLOBAL_CONFIG['random_seed'])
            n_ood = min(config['n_ood_samples'], len(ood_texts))
            ood_texts = list(np.random.choice(ood_texts, n_ood, replace=False))

            print(f"✅ OOD数据加载完成: {len(ood_texts)}个样本")

            # ID数据：使用Banking77的子集作为替代
            print("\n获取ID数据（使用Banking77子集作为替代）...")
            banking_ds = load_dataset("PolyAI/banking77", split="train")
            id_texts = list(banking_ds['text'][:config['n_id_samples']])

            print(f"✅ ID数据加载完成: {len(id_texts)}个样本")

            print(f"\n✅ ROSTD加载完成（GitHub源）:")
            print(f"   ID样本: {len(id_texts)}")
            print(f"   OOD样本: {len(ood_texts)}")
            print(f"   OOD类型: 人工撰写的多样化OOD")

            return id_texts, ood_texts, {
                'dataset': 'ROSTD',
                'source': 'GitHub + alternative ID',
                'ood_type': 'diverse human-authored',
                'note': 'Using alternative ID dataset due to availability'
            }
            ...
            # 方式3：使用其他意图数据集模拟
            # ==================================================================
            print("尝试方式3：使用SNIPS数据集模拟ROSTD场景...")
            try:
                # 使用SNIPS数据集
                ...
                id_labels = unique_labels[:n_id_classes]
                ood_labels_set = unique_labels[n_id_classes:]
...
def load_toxigen_data(config):
    ...
    try:
        # ToxiGen数据集
        dataset = load_dataset("skg/toxigen-data", "train", split="train")

        np.random.seed(GLOBAL_CONFIG['random_seed'])

        # 根据toxicity_ai字段分离
        toxicity_scores = np.array(dataset['toxicity_ai'])

        # ID：低毒性（toxicity < 0.5）
        id_mask = toxicity_scores < 0.5
        id_indices = np.where(id_mask)[0]

        # OOD：高毒性（toxicity >= 0.5）
        ood_mask = toxicity_scores >= 0.5
        ood_indices = np.where(ood_mask)[0]
        ...
        id_texts = [dataset['text'][i] for i in selected_id]
        ood_texts = [dataset['text'][i] for i in selected_ood]

        print(f"✅ ToxiGen加载完成:")
        print(f"   ID样本: {len(id_texts)} (低毒性陈述)")
        print(f"   OOD样本: {len(ood_texts)} (高毒性陈述)")
        print(f"   OOD类型: 领域偏移（有毒内容）")
        ...
        print(f"⚠️ ToxiGen加载失败: {e}")
        print("尝试使用替代方案：Hate Speech数据集...")

        try:
            # 替代方案：使用hate speech数据集
            ...
            print(f"✅ ToxiGen替代方案加载完成（Hate Speech18）:")
            print(f"   ID样本: {len(id_texts)} (非仇恨言论)")
            print(f"   OOD样本: {len(ood_texts)} (仇恨言论)")

            return id_texts, ood_texts, {
                'dataset': 'ToxiGen-alternative',
                'source': 'hate_speech18',
                'id_type': 'non-hate speech',
                'ood_type': 'hate speech (domain shift)'
            }
